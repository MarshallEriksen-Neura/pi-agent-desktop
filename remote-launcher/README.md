...
- `--capabilities`: takes no payload and writes one JSON document to stdout. Launcher revision 10 advertises `run-v1`, `preflight-v1`, provider sync, detached task/attach, workspace read/write, repository review (`repository-read-v1`), safe repository mutations (`repository-write-v1`), and four independently gated PI-management capabilities: `pi-packages-read-v1`, `pi-packages-mutate-v1`, `pi-skills-read-v1`, and `pi-skills-mutate-v1`. Repository reads and writes are independently gated. `launcherRevision` identifies functional launcher builds; `statusVersion` changes only when the detached `status.json` schema changes. Detached task status persists the authoritative Pi busy state, and keyed detached sends are durably recorded by the task supervisor before they are forwarded, so reattach and ambiguous-send retries remain fail-closed.
...
- `--repository`: accepts one base64-encoded repository protocol v1 request. `status` returns porcelain v2 metadata for the workspace; `diff` returns one staged or unstaged file diff after revalidating the declared repository root. Revision 7 also supports generation-checked single-file `stage`/`unstage` and staged-only `commit`. Every mutation revalidates the workspace/repository identity, authoritative generation, operation/conflict state, and returns a fresh status snapshot. Paths are passed to fixed Git arguments without shell interpolation; commits are non-interactive and never push. Git has a 30-second deadline, all output is bounded, and oversized diffs are returned with `truncated: true`.

Repository mutation replies are deliberately fail-closed. Success includes the authoritative `repoRoot`, porcelain status, current repository operation, and (for commits) `commitOid`. Failures include a stable `reason`, optional `detail`, and `applied`; `applied: true` means Git wrote successfully but the authoritative refresh failed. Expected reasons include stale generation, repository drift, conflicts, an active Git operation, an index lock, empty staged content, and invalid or empty commit messages.
