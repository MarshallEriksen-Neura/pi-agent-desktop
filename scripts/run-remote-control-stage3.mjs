...
const targetDir = path.join(root, ".tmp", "remote-control", "stage3-target");
fs.mkdirSync(targetDir, { recursive: true });

const tsc = path.join(root, "node_modules", "typescript", "bin", "tsc");
if (!fs.existsSync(tsc)) {
  console.error(`Stage 3 evaluator: missing TypeScript compiler at ${tsc}`);
  process.exit(1);
}
...
console.log(
  "remote-control Stage 3: contracts, identity/pairing/device security, full Rust tests, MSRV and hygiene checks passed",
);
