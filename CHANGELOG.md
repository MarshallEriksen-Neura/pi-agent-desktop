...
### Added
- 仓库检查器的 Push 按钮现在显示待推送 commit 数量，并提供准确的单复数无障碍标签。

### Changed
- **SSH 执行目标现在与本地工作区共用完整布局能力**。远程会话不再被强制进入仅聊天界面，可以使用编辑器、Agent 面板、Zen / Work 模式、命令面板及对应快捷键。
- **终端抽屉可以扩展到完整工作区高度**。窗口暂时变小时只限制当前显示高度，不再永久截断用户保存的高度偏好；窗口恢复后会回到原先尺寸。
- 使用自定义背景图时，MCP 页面和纸张卡片改用半透明混合表面与背景模糊，不再以完全不透明背景遮住图片。

### Fixed
- **修复远程 Agent 省略可选浏览起始目录时 readiness 被错误判定失败、导致无法保存的问题**。只有用户明确留空目录时，`workspace: skipped` 才是非阻断状态；真正的 SSH、launcher、Node.js、Pi 或指定目录失败仍会阻止保存。
- 缺少默认 Pi 凭据现在作为非阻断建议显示。界面会明确说明保存仍然可用，且远程 Pi 通过环境变量或其他认证机制运行时可以忽略该警告。
- readiness 的成功、建议警告和阻断失败现在分别呈现；修改主机、launcher、Pi 路径或浏览目录后会清除旧报告，避免使用过期检查结果。
- 修复没有在开始事件中携带路径的 `insert` / `replace` 编辑结果未进入 diff 与最近修改处理链的问题；现在可从工具结果或统一补丁头恢复目标路径。
- 修复后台任务及历史 transcript 恢复后编辑工具的 `+/-` diff 徽章丢失问题。

### Internal
- 浏览器 Mock 与 Rust readiness 聚合规则保持一致，并增加可选工作区、凭据 warning、前置失败和异常 `skipped` 的回归覆盖。
- 新增 SSH 布局一致性、终端高度、编辑结果路径、历史 diff 指标及 Repository Push 数量测试。
...
### Added
- **终端抽屉升级为真正的多标签 PTY 终端**。本地 Shell 与 SSH 终端都运行在独立原生 PTY 中，可以并行保留、切换、重命名和关闭，抽屉隐藏后会话仍继续运行；每个标签保存创建时的工作目录与执行目标快照，避免项目或 SSH 配置切换后把已运行终端悄悄迁到另一处
  - 最多同时保留 8 个标签；本地标签支持 xterm 自适应尺寸、Unicode 11、WebGL 回退、链接、复制粘贴与文件拖放。浏览器预览继续使用原有 Pi RPC 终端，不伪装成本机 PTY
  - 本地与 SSH 终端使用 generation 隔离迟到的数据、退出与写入事件；关闭标签、应用退出和启动失败都会回收子进程与 PTY 句柄，终端字节不会经过 Pi JSONL RPC 通道
- **设置页新增本地终端 Shell Profile**。Windows 的 Auto 模式依次尝试 PowerShell 7、Windows PowerShell 和命令提示符，也可以选择不带参数的绝对可执行文件路径；路径无效或启动失败会安全回退到 Auto，并在终端中明确提示。该设置只影响之后新建的本地标签，不改变运行中的标签、SSH 终端或 Pi 工具命令
- **长文本现在可以作为独立文档编辑与查看**。粘贴达到 800 个 Unicode 字符或 12 行时，输入框收起为紧凑引用卡片，可以在完整编辑器中修改，再附加一条简短指令一起发送；消息气泡同样以紧凑卡片显示，并可打开全文、复制原文。弹窗支持焦点恢复、Esc 关闭和 Tab 焦点循环

- **模型思考能力现在可按实际支持范围配置和选择**。模型页可以覆盖 reasoning 开关，并为每个思考等级继承、映射自定义值或标记不支持；输入区从 Pi 动态读取当前模型可用等级，不再展示无效选项。模型与思考等级写入串行执行，迟到的能力快照不会覆盖最新选择，Pi 对不支持等级的实际回退会同步回界面和持久化偏好
### Changed
- **远程项目切换现在同步切换执行作用域**。进入新的 SSH 工作目录时会先读取目录、清除只属于旧 cwd 的 detached task 绑定，再原子更新会话目标、最近项目和可见文件树；任一步失败都会尝试恢复之前的 binding，避免界面与实际 Pi 工作目录分叉
- **默认深色背景由纯黑改为 `#1c1c1e`**，抬升层与半透明材质同步调整为 `#2c2c2e` 系列，保留卡片、输入区和浮层之间的层级。用户保存的自定义背景仍优先于默认主题
- 插件与 Skills 的远程数据读取期间改为结构化 skeleton；目标不支持读取能力或加载失败时才显示明确的 unavailable/error 状态，不再把真实错误伪装成永久加载

### Fixed
- **修复终端标签关闭或重连后迟到事件污染新实例的问题**。前后端共同校验 session generation，启动竞态中的失败方会立即终止并回收，stop 后排队写入也不会再送到已关闭会话
- **修复远程项目切换可能接回旧目录任务的问题**。只要 SSH cwd 发生变化，`remoteTaskId` 与 pending 状态都会被丢弃，新项目不会错误附着到旧项目进程

### Internal
- 本地终端、SSH 终端与 Pi 工具执行保持三条独立进程/传输边界；PTY 输入、尺寸、会话 ID、输出分块、cwd 与自定义 Shell 路径均有后端校验和容量上限
- 新增长文本、终端标签、Shell Profile、desktop terminal IPC 与远程项目切换回归测试；发布范围、验证证据和剩余平台风险记录在 [release-audit.md](release-audit.md)
- 发布工作流现在会等待桌面矩阵和 Android APK 都完成后再生成 updater manifest 并发布 Release，避免 Android 产物晚到已发布版本
