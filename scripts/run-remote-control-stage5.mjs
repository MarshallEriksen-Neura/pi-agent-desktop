...
const targetDir = path.join(root, ".tmp", "remote-control", "stage5-target");
fs.mkdirSync(targetDir, { recursive: true });

const tsc = path.join(root, "node_modules", "typescript", "bin", "tsc");
if (!fs.existsSync(tsc)) {
  console.error(`Stage 5 evaluator: missing TypeScript compiler at ${tsc}`);
  process.exit(1);
}
...
console.log(
  "remote-control Stage 5: dedicated runtime, fake-Pi lifecycle, bounded lanes, MSRV and hygiene checks passed",
);
