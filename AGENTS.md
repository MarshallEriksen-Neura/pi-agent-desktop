...
3. 严禁执行全量测试或全量 build。只能运行与本次改动直接相关的单个测试文件、单个测试用例或最小范围检查；不得调用会聚合整个测试套件或构建整个项目的命令/脚本（例如 `scripts/run-backend-tests.mjs`、无筛选的 `cargo test`、全量 `npm run build`）。只有用户明确要求并授权发布级验证时才能例外执行。
