...
const targetDir = path.join(root, ".tmp", "remote-control", "stage4-target");
fs.mkdirSync(targetDir, { recursive: true });

const tsc = path.join(root, "node_modules", "typescript", "bin", "tsc");
if (!fs.existsSync(tsc)) {
  console.error(`Stage 4 evaluator: missing TypeScript compiler at ${tsc}`);
  process.exit(1);
}
...
console.log(
  "remote-control Stage 4: contracts, gateway auth/body/TLS tests, full Rust tests, MSRV and hygiene checks passed",
);
